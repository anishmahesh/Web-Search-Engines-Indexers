 ...
 */
public class IndexerInvertedCompressed2 extends Indexer implements Serializable {
    ...
    private Map<String,Integer> _lastOffest = new HashMap<>();

    private Query queryObject = null;


    public IndexerInvertedCompressed2(Options options) {
        super(options);
    ...
    public void compressMergedFiles(String dir) throws IOException {
        ...
        for (File file : fileNames) {
            if (file.isFile() && !file.isHidden() && !file.getName().toLowerCase().contains("object.idx")) {
                ...
                ObjectOutputStream oOPS = new ObjectOutputStream(new FileOutputStream(dir + "/index-comp-part-"+fileNum+".tsv"));
                try (BufferedReader br = new BufferedReader(new FileReader(file))) {
                    ...
                    while ((line = br.readLine()) != null) {
                        ...
                        count++;
                        while (sc.hasNext() && count<100000) {
                            int scn = sc.nextInt();
                            VByteEncoder.encode(oOPS,(long) scn);
                            oOPS.write("\t".getBytes());
                            //Vector<Byte> vb = IndexCompressor.vByteEncoder(scn);

              /*
              for (Byte singleByte : vb) {
    ...
    */
    private void processDocument(String content, DocumentIndexed doc) {
        ...
        while (s.hasNext()) {
            ...
            if (!termOccurenceMap.containsKey(term)) {
                ...
                int currentPointer = lastTermIndex.get(term);
                occurence.add(offset-currentPointer);
                lastTermIndex.put(term,offset-currentPointer);
            }
            offset++;
        }
    ...
    public void loadIndex() throws IOException, ClassNotFoundException {
        ...
        IndexerInvertedCompressed2 loaded = (IndexerInvertedCompressed2) reader.readObject();
        ...
        if (indexNo < indexFiles.length) {
            ...
            try {
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    while (sc2.hasNext()) {
                        temp.add(Byte.parseByte(sc2.next()));
                    }
                    int termId = IndexCompressor.vByteDecoder(temp).get(0);
                    line = reader.readLine();
                    ...
                    _postings.put(termId, IndexCompressor.vByteDecoder(termPostingList));
